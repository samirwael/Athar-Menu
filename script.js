...
const FIREBASE_CONFIG = {
  apiKey: "AIzaSyARpchwvkweA04wDhAnW5lK1fwjqrWGiSM",
  authDomain: "athar-menu.firebaseapp.com",
  databaseURL: "https://athar-menu-default-rtdb.firebaseio.com",
  projectId: "athar-menu",
  storageBucket: "athar-menu.firebasestorage.app",
  messagingSenderId: "280208292051",
  appId: "1:280208292051:web:fcf11ae0bbaaa9645bfc26"
};

// Firebase REST helpers (no SDK needed — pure fetch)
const FB_BASE = FIREBASE_CONFIG.databaseURL + "/menu";

async function fbGet(path) {
  const res = await fetch(`${FB_BASE}/${path}.json`);
...
// Safe key encoding (Firebase keys can't have . $ # [ ] /)
function fbKey(str) {
  return str.replace(/[.$#[\]/]/g, "_");
}

function fbKeyDecode(str) { return str; }

// Read a whole node as { key: value } map
async function fbGetAll(node) {
  const data = await fbGet(node);
  if (!data || typeof data !== "object") return {};
  // Firebase may return array-like — normalise
  const out = {};
  Object.entries(data).forEach(([k, v]) => { out[k] = v; });
  return out;
}

// ── LIVE SYNC — Firebase onValue via SSE ──
let _fbListenerActive = false;
function startFirebaseLiveSync() {
  if (_fbListenerActive) return;
  _fbListenerActive = true;
  const url = `${FB_BASE}.json?limitToLast=1`;
  // Poll every 5 seconds as a lightweight sync (SSE needs auth in some configs)
  setInterval(() => { refreshMenuData(); }, 5000);
}

// ── MERGE ALL DATA FROM FIREBASE ──
async function buildMenuData() {
  const [imgOverrides, detOverrides, addedItems, deletedKeys, customCats] = await Promise.all([
    fbGetAll("image_overrides"),
    fbGetAll("item_overrides"),
    fbGetAll("added_items"),
    fbGetAll("deleted_items"),
    fbGetAll("custom_categories")
  ]);

  const base = menuDataDefault
    .filter(item => !deletedKeys[fbKey(item.cat + "::" + item.name)])
    .map(item => {
      const key = fbKey(item.cat + "::" + item.name);
      const merged = { ...item };
  ...
  const baseCatKeys = Object.keys(catColors);
  const customCatKeys = Object.keys(customCats).filter(c => !baseCatKeys.includes(c));
  return { items: [...base, ...added], customCatKeys };
}

async function initMenuData() {
  try {
    const { items, customCatKeys } = await buildMenuData();
    menuData = items;
    rebuildTabs(customCatKeys);
  } catch (e) {
...
// ── DYNAMIC TABS ──
function rebuildTabs(customCatKeys) {
  const tabsEl = document.getElementById("menuTabs");
  if (!tabsEl) return;
  const baseCats = Object.keys(catColors);
  const allCats = ["All", ...baseCats, ...customCatKeys];

  tabsEl.innerHTML = allCats.map((c, i) =>
    `<button class="tab${activeCategory === c ? " active" : ""}" data-cat="${c}">${c}</button>`
  ).join("");

  // Re-inject dividers and re-bind events
...
// ── TAB SWITCHING (initial placeholder — real tabs built by rebuildTabs) ──
function initTabs() {
  // Tabs are built dynamically by rebuildTabs() after IndexedDB loads.
  // This is a no-op to preserve call-site compatibility.
}
